...
def load_cyclegan_model():
    ...
    if '--name' not in sys.argv:
        sys.argv += ['--name', '2.hybrid_restormer']
    if '--gpu_ids' not in sys.argv:
        sys.argv += ['--gpu_ids', '0']
    if '--model' not in sys.argv:
        sys.argv += ['--model', 'test']
    if '--netGab' not in sys.argv:
        sys.argv += ['--netGab', 'hybrid_restormer_6blocks']
...
if __name__ == '__main__':
    ...
    output_folder = f'./results/test_2.hybrid_restormer'  # 输出文件夹路径
